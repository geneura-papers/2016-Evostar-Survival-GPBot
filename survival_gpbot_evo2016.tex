...
\author{A.Fern\'andez-Ares \and P.Garc\'ia-S\'anchez \and
  A.M Mora \and P. A. Castillo \and J. J. Merelo \inst{1}}
%\thanks{NoInstitute}}
...
i.e. GP is able to create new sets of behavioral rules meanwhile while GA is devoted to optimize the parameters of previously designed rules. This technique has yielded excellent results in previous works related with agent generation in videogames \cite{GarciaGP14,EsparciaGP2013}.
...
\item {\em Attack Wealthiest (Neutral|Enemy|NotMy) Planet}: The objective is the planet with the highest growth rate.
...
%  \FOR{1 to N}
    ...
    \STATE son1,son2 $\leftarrow$ crossover(winner1,winner2);
...
Thus, to do a fair comparison with that method and the results it yields, the termination criteria in SurvivalBot has been set to 8000 battles (therefore, 4000 generations/iterations), since GPBot considered 32 individuals * 5 combats per evaluation * 50 generations = 8000 combats. The five maps are representative of different distributions and sizes of planets.
...
Selection & Joust (2-tournament) \\ \hline
...
This figure shows how there exist an increasing performance in the best (and also in average) individuals during the runs. Moreover, a slightly noisy factor is present, but the oscillations are not as striking as in previous approaches \cite{Genebot_JCST}.
...
\end{center}
\caption{Histogram of number of victories against GPBot of all the individuals in initial and final populations in the 30 battles (10 maps and 3 combats).} 
...
one run. It is interesting how the age has an upper limit that
...
objectives. We will do this next.
